						...
						function selectbox() {
							...
							function makeList() {
								for (i = 0, len = option.length; i < len; i++) {
									...
									if (option.eq(i).attr('listType') !== undefined) listType = option.eq(i).attr('listType');
									if (option.eq(i).attr('serviceTypeId') !== undefined) serviceTypeId = option.eq(i).attr('serviceTypeId');
									if (option.eq(i).attr('vendorId') !== undefined) vendorId = option.eq(i).attr('vendorId');
									if (option.eq(i).attr('id') !== undefined) listId = option.eq(i).attr('id');


									li = '<li class="' + liClass + optionClass +'"' + 'listType="' + listType + '" serviceTypeId="' 
									+ serviceTypeId + '" vendorId="' + vendorId + '" id="' + listId + '" onclick="sortFun(this);">'+ option.eq(i).text() +'</li>';
									// если есть optgroup
