...
private
	...
	def self.tariff_params(params, user, service)
		tariff_template = TariffTemplateManager.get(params[:tariff][:tariff_template_id])

		t_p = {
			   tariff_template_id: 	params[:tariff][:tariff_template_id],
		...
		unless params[:vendor_id].to_i != 0 || params[:vendor_id]
			t_p.merge!(owner_type: "User", owner_id: user.id)
		else
			t_p.merge!(owner_type: "Vendor", owner_id: params[:vendor_id])
		end
